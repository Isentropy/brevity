...
the following keywords have special functions
...
### Clear Pre-Processor Params (except for all uppercase)
```
clearParams
```
Brevity has no subroutines, and data is usually passed in pre-processor symbols. In order to compose steps, we need to clear out the last steps. The convention is that uppercase names are global and non-uppercase names are local. This clears the pre-processor symbols that are not all uppercase (ie the local ones).
...
### Switch checked/unchecked arithmatic. checkedArithmatic is default
```
uncheckedArithmatic
checkedArithmatic
```
