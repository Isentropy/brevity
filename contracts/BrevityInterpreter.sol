...
contract BrevityInterpreter is EIP712, IBrevityInterpreter {
    ...
    function run(Brevity.Program calldata p) public payable override {
        Brevity._run(p.memSize, p.instructions, p.quantities);
    }
    ...
    function runMeta(
        ...
        bytes32 structHash = keccak256(abi.encode(Brevity._PROGRAM_TYPEHASH, p.memSize, Brevity._encodeInstructionsArray(p.instructions), Brevity._encodeQuantityArray(p.quantities), nonce));
