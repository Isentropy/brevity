...
export function erc20Approve(erc20, spender, amount) {
    return `CALL ${erc20}.${SELECTOR_IERC20_approve}(${spender}, ${amount})`;
}
export function erc20transfer(erc20, to, amount) {
    return `CALL ${erc20}.${SELECTOR_IERC20_transfer}(${to}, ${amount})`;
}
export function requireERC20Increase(brevityScript, erc20Address, requiredIncrease, tmpVarName = null) {
...
export function defineFunctionFragment(brevityScript, frag) {
    const pre = `${frag.name} := ${frag.selector}\n`;
    return pre + brevityScript;
}
export function transferAllErc20(erc20, to) {
...
export function transferAllEth(to) {
    const noTransfer = `noTransfer_${genRandomHex(6)}`;
...
export function requireEthIncrease(brevityScript, requiredIncrease, tmpVarName = null) {
...
function test() {
    ...
    const parser = new BrevityParser({ maxMem: 100 });
